...
function filter(obj,options) {
    ...
    recurse(src,{},function(obj,key,state){
        ...
        for (let flag of options.flags) {
            if ((options.checkTags == false && (obj[key] && (obj[key][flag] == true || options.flagValues.includes(obj[key][flag])))) || (options.checkTags && (obj[key] && obj[key].tags && Array.isArray(obj[key].tags) && obj[key].tags.includes(flag)))) {
                if (options.inverse) {
                    if (options.strip) {
                        delete obj[key][flag];
                    }
